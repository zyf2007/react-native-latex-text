...
A lightweight, high-performance React Native component that supports mixed typesetting of text and LaTeX formulas, features customizable formula delimiters, enables line breaks via \n, automatically calculates formula line height, renders content in react-native-svg based on MathJax SVG, comes with built-in caching, and has no native dependencies (runs directly in Expo Go).
...
The `MathJaxRenderer` component creates a hidden 1x1 WebView at the root level to load the MathJax library and render formulas. It sends the SVG-XML string back to React Native via `postMessage`, using react-native-svg components for high-performance display.
...
<MathText
  content='Block Formula: \[\int_0^2 x^3 dx\]\nInline Formula: Calculate $E = \frac{F}{q}$. If $q = 2×10^{-6}C$ and $F = 4×10^{-3}N$, what is $E$?'
  textColor='#FFFFFF'
/>
